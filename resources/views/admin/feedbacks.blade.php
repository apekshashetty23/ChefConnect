...
.page-title{font-weight:600;color:#ff5c5c;margin-bottom:25px}
.table-card{background:#fff;border-radius:20px;padding:25px;box-shadow:0 8px 25px rgba(0,0,0,.08)}
table thead{background:#ff5e62;color:#fff}
table th,table td{vertical-align:middle}
.badge-user{background:#ffe3e3;color:#dc3545;font-weight:500;padding:6px 10px;border-radius:8px}
</style>
</head>
...
<a href="{{ route('admin.feedbacks') }}" class="active"><i class="fa fa-comment"></i>Feedbacks</a>
...
<td><span class="badge-user">{{ $feedback->name }}</span></td>
<td>{{ $feedback->email }}</td>
<td>{{ $feedback->message }}</td>
</tr>
@empty
<tr>
<td colspan="4" class="text-center text-muted py-4">No feedbacks available yet</td>
</tr>
@endforelse
</tbody>
</table>
